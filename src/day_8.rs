...
impl Nodes {
    ...
    fn starting_points(&self, character: char) -> Vec<Node> {
        self.nodes
            .iter()
            .filter(|(k, v)| v.value.chars().last().unwrap() == character)
            .map(|(k, v)| v.clone())
            .collect()
    }
}
...
fn next<'a>(
    points: &mut (String, &(String, String)),
    direction: char,
    nodes: &'a BTreeMap<String, (String, String)>,
) -> Option<(&'a String, &'a (String, String))> {
...
fn compare(points: &Vec<String>) -> bool {
    match points
        .iter()
        .try_for_each(|x| match x.chars().last().unwrap() == 'Z' {
            true => Ok(()),
            false => Err(()),
        }) {
        Ok(_) => true,
...
mod tests {
    ...
    fn find_nodes_end_with_a() {
        ...
        let nodes: BTreeMap<String, Node> = nodes
            .into_iter()
            .map(|(k, v)| Node {
                value: k,
                left: v.0,
